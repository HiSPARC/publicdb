...
def maakconfig(request):
	...
	for mach in machines:
		...
		services =  PcMonitorService.objects.filter(pc=mach)
		for serv in services:
			if serv.monitor_service.nagios_command.split('!')[0] == 'check_nrpe':
				# kijken of er overrides zijn gedaan op de standaard waardes
				if serv.override_min_warning:
					minwarning = serv.override_min_warning
				else:
					minwarning = serv.monitor_service.min_warning
				if serv.override_max_warning:
					maxwarning = serv.override_max_warning
                                else:
                                        maxwarning = serv.monitor_service.max_warning
                                if serv.override_min_critical:
                                        mincritical = serv.override_min_critical
                                else:
                                        mincritical = serv.monitor_service.min_critical
                                if serv.override_max_critical:
                                        maxcritical = serv.override_max_critical
                                else:
